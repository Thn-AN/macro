...
; Main inputs and routing by .zovc 
; Contributions to user customization, support of multiple aspect ratios, and error handling by Orchrist 
; Assembly of the framework for this script by antrament


; Comments marked with Z- are specific descriptions or instructions for adjusting certain variables to your needs.

;; Required settings:
; - Windowed Fullscreen, cap to 30 fps
; - Fully default controller settings (as in reset them and then dont touch them)
...
;; Variables for easy editing by user
;;/////////////////////////////////////////////////////////////////////////////////////////////////////////
; Z- These may be edited to tweak your paths and menu navigation, if you need to. Default values are listed in (Parentheses) to the right
	global initialaim 	:= 2465		; (2465) How long to move the reticle towards the right when aiming for the run path
    global prearcsleep 	:= 3860		; (3860) How long you run forward before starting the arc when running to the chest
    global arcsleep 	:= 4250		; (4250) How long you arc to the left towards the chest
    global postarcsleep := 1000		; (1000) How long you run forward after the arc is completed. This should end up so you can open the chest
...
; Z- ↓ Fallback timing. The most common value across ratios is (2100). 
global fasttravelsleep	:= 2100		; Z- Can be edited to fix cursor missing The Landing's map node. Bigger number = more left.

global orbitlauncherright 	:= 990	; (990)  How long cursor moves horizontally when launching The Landing from orbit
global orbitlauncherdown 	:= 990	; (990)  How long cursor moves vertically when launching The Landing from orbit
; Z- ↑↑ These two can be edited to fix the cursor missing the launch button. Bigger number = more right/down respectively.


; Z- If you want or need to check what size the script is reading your game window to be, 
;	delete the semi-colon on the line below, then save and reload the script.
;MsgBox, , , D2 is scaled at %D2Width% x %D2Height%


; Z- ↓ These are ratio detections that can be adjusted to your specific needs. The detections really only serve as a starting place for the timings. 
;		 Please do share your adjustments here with @Zovc, so that I may set better default timings for aspect ratios!

	;; 16:9 resolution (1920 x 1080, 2560x1440)
if ((D2Width/D2Height) == (16/9)) {
	global fasttravelsleep := 2100		; How long your cursor scrolls to click on the Landing fast travel
}
	;; 16:10 resolution (1920 x 1200)
if ((D2Width/D2Height) == (16/10)) {
	global fasttravelsleep := 2100		; How long your cursor scrolls to click on the Landing fast travel
}
	;; 32:9 resolution (5140 x 1440)
if ((D2Width/D2Height) == (32/9)) {
	global fasttravelsleep := 2100		; How long your cursor scrolls to click on the Landing fast travel
}
	;; 21:9 resolution ( x )
if ((D2Width/D2Height) == (21/9)) {
	global fasttravelsleep := 1500		; How long your cursor scrolls to click on the Landing fast travel
}
	;; 24:10 resolution (3840 x 1600)
if ((D2Width/D2Height) == (24/10)) {
	global fasttravelsleep := 1500		; How long your cursor scrolls to click on the Landing fast travel
}
	;; 43:18 resolution (3440 x 1440)
if ((D2Width/D2Height) == (43/18)) {
    global fasttravelsleep := 1500		; How long your cursor scrolls to click on the Landing fast travel
}

;Error handler for if screen ratio is not currently supported
;;/////////////////////////////////////////////////////////////////////////////////////////////////////////
if ((D2Width/D2Height) != (16/9) && (D2Width/D2Height) != (16/10) && (D2Width/D2Height) != (32/9) && (D2Width/D2Height) != (24/10) && (D2Width/D2Height) != (43/18) ){
    MsgBox, 262404, , 
	(
Your Screen aspect ratio is either not detected or not supported.
Make sure you are running in Windowed Fullscreen and with the correct settings.

The script will run a fallback setting, you can edit this fallback to fit your needs near the top of the script in notepad.
...
; i thought corrupt downloads were a thing of the past but i guess not with this apparently ultra fragile file
	...
	( LTrim Join
	  ...
	  BVM7EXgAlfOtMaD0RSkAfoqqJtpbhzh7MluBoe8moFb8QQrdr09EsEO8cxLDhN2x+LEUnCprAIK9toitk9hMbBHfxqf3008KbmrC
	...
	( LTrim Join
	  ...
	  EuL9KW0NwzltC9M4jYbVnLaH1SpKO8I0TjvBIk7joJzTeHjfh9IucJjTbnDdSGkPmM78Q7AB300R/i5cNIqYLhVWh1O6TJgfrgBl
...
class ViGEmTarget {
	...
	__New(){
		...
		if (this.Instance.OkCheck() != "OK"){
			msgbox,4,, The .dll failed to load!`n`nPress yes if you need help!
			IfMsgBox, Yes
			{
				Run, https://discord.gg/KGyjysA5WY
				exitapp
...
class ShinsOverlayClass {
	...
	;									Width .............	w[number]				: Example > w300			(Default: this.width)
	;									Height ............	h[number]				: Example > h300			(Default: this.height)
	...
	CacheImage(image) {
		...
		DllCall("Gdiplus\GdipBitmapLockBits", "Ptr", bm, "Ptr", &r, "uint", 3, "int", 0x26300A, "Ptr", &bmdata)
...
UpdateOverlay:
	if WinExist("ahk_exe destiny2.exe") 
	{
		...
		font_size := h/38 ; auto scale overlay text by the height of the destiny 2 window
		if (overlay.beginDraw())  ; if we can draw the overlay... (is d2 active? did the class load correctly?)
		{
			inputs := "| F8 - Close Script | F9 - Setup Info | Pale Heart Chest Farming - Zov edition | Thrallway.com"
			switch status 
			{
				case 0: ; Z- script is not started
					overlay.drawText("| F4 - Start Script " . inputs, 10, 0, font_size, 0xFFFFFFFF, "Courier", "olFF000000")
				case 1: ; Z- script is running and counting cycles
					overlay.drawText("| You may now tab out | F5 - Stop Script " . inputs . "`n| " . count . " total chests attempted |", 10, 0, font_size, 0xFFFFFFFF, "Courier" , "olFF000000")
				case 2: ; Z- script is running
					overlay.drawText("| You may now tab out | F5 - Stop Script " . inputs, 10, 0, font_size, 0xFFFFFFFF, "Courier" , "olFF000000")
			}
...
; Z- Begins the overall looping route and UI navigation
Start_Route: 
	
	Loop
	{

		if(inOrbit == 0){ ; Z- allows F3 to start the route from orbit (hopefully)
			360Controller.Buttons.Back.SetState(true)	; Ghost menu
			PreciseSleep(100)							; ↑
			360Controller.Buttons.Back.SetState(false)	; ↑
			PreciseSleep(100)
			360Controller.Buttons.Y.SetState(true)		; Return to orbit
			PreciseSleep(3200)							; ↑
			360Controller.Buttons.Y.SetState(false)		; ↑
			PreciseSleep(15000)
		}	
		360Controller.Buttons.Back.SetState(true)	; Open Destinations tab
		PreciseSleep(100)							; ↑
		360Controller.Buttons.Back.SetState(false)	; ↑
		PreciseSleep(1000)
		...
		360Controller.Axes.LY.SetState(50)			; ↑
		PreciseSleep(600)
		360Controller.Buttons.A.SetState(true)		; Select Pale Heart
		PreciseSleep(100)							; ↑
		360Controller.Buttons.A.SetState(false)		; ↑
		PreciseSleep(1000)
		360Controller.Axes.LX.SetState(0)			; Cursor to left
		PreciseSleep(fasttravelsleep)				; ↑ 2120 default
		360Controller.Axes.LX.SetState(50)
		
	PreciseSleep(100)
	360Controller.Axes.LY.SetState(30)
	PreciseSleep(100)	; Z- bumps down the map cursor because its a little high for the LZ node sometimes
	360Controller.Axes.LY.SetState(50)

		PreciseSleep(200)				; ↑
		360Controller.Buttons.A.SetState(true)		; Select Landing
		PreciseSleep(100)							; ↑
		360Controller.Buttons.A.SetState(false)		; ↑
		PreciseSleep(100)
		360Controller.Axes.LX.SetState(100)			; Cursor to the right
		PreciseSleep(orbitlauncherright)
		360Controller.Axes.LX.SetState(50)			; Cursor stops
		PreciseSleep(100)
		360Controller.Axes.LY.SetState(21)			; Cursor down, slowly
		PreciseSleep(orbitlauncherdown)
		360Controller.Axes.LY.SetState(50)			; Cursor stops
		PreciseSleep(100)
		360Controller.Buttons.A.SetState(true)		; Hits Launch button
		PreciseSleep(100)							; ↑
		360Controller.Buttons.A.SetState(false)		; ↑
		PreciseSleep(30000)
		...
		Loop 40
		{
			360Controller.Buttons.Back.SetState(true) 	; Open Map
			PreciseSleep(1000)							; ↑
			360Controller.Buttons.Back.SetState(false)	; ↑
			PreciseSleep(250)
			360Controller.Axes.LX.SetState(0)			; Stick to left
			PreciseSleep(fasttravelsleep)				; ↑ (2140)default
			360Controller.Axes.LX.SetState(50)			; ↑
		...
		360Controller.Axes.LY.SetState(50)
			...
			PreciseSleep(200)
			360Controller.Buttons.A.SetState(true)		; Fast travel to The Landing
			PreciseSleep(1100)							; ↑
			360Controller.Buttons.A.SetState(false)		; ↑
			PreciseSleep(10000)
	;/*		; Z- just here to mark and comment out the running portion for easy map nav testing
			360Controller.Axes.RX.SetState(10)			; Initial left turn after spawning
			PreciseSleep(565)							; ↑
			360Controller.Axes.RX.SetState(50)			; ↑
			PreciseSleep(100)
			360Controller.Buttons.LS.SetState(true)		; Sprint down
			PreciseSleep(50)
			360Controller.Axes.LY.SetState(100)				; Walk forward
			PreciseSleep(7500)								; ↑
			360Controller.Axes.LY.SetState(50)				; Stop Walking forward
			PreciseSleep(50)
			360Controller.Buttons.LS.SetState(false)	; Sprint release
			PreciseSleep(100)
			360Controller.Axes.RX.SetState(90)			; Turn right, toward chest
			...
			360Controller.Axes.RX.SetState(18)					; Start left arc
			PreciseSleep(arcsleep)									; ↑
			360Controller.Axes.RX.SetState(50)					; Stop left arc
			PreciseSleep(postarcsleep)
			360Controller.Axes.LY.SetState(50)				; Stop Walking forward
			PreciseSleep(50)
			360Controller.Buttons.LS.SetState(false)	; Sprint release
			PreciseSleep(50)
			360Controller.Buttons.X.SetState(true)		; Loot chest
			PreciseSleep(1350)							; ↑
...
; Z- Provides a rough explanation of the required settings and common issues.
Info_Hub:
MsgBox, , Required & Important Info, 
(Start this script from patrol in the Pale Heart.
You may need to clear Pathfinder objectives, if any, that the cursor hits while moving across the map, and any objectives related to opening chests in The Landing.

Use Windowed Fullscreen, disable Vsync, and cap your frames at 30 fps.
Reset your controller settings and binds.
No other settings need to be changed.

Disconnect any controllers from your machine.
Avoid equipping weapons or exotic armors that affect movement speed.
Avoid using the Traveler's Blessing of Transference (Amplified by sprint).
Your Mobility stat does not matter for this script.
...
; Z- ↓ Blocks certain inputs after the script has started
#If, status == 0
...
F3:: 
; Z- Currently set to start the full script from orbit, rather than from the ground.
	status = 1
	inOrbit = 1
	gosub, Start_Route
Return
...
#If
; Z- ↑ End of the section containing blocked inputs
