...
global prearcsleep 	:= 400			; (4700) How long you run forward before starting the arc when running to the chest
...
global orbitlauncherright := 800	; ^  How long cursor moves to the right when launching the landing from orbit
...
;; Antra's bag of tricks
	...
	;; ViGE Wrapper DLL Creation
		if FileExist(A_ScriptDir "\vigemwrapper.dll") {
		} else
		{
		...
		; i thought corrupt downloads were a thing of the past but i guess not with this apparently ultra fragile file
			...
			File := FileOpen(A_ScriptDir "\vigemwrapper.dll", "w")
	...
	;; ViGEm Bus Setup
		...
		class ViGEmWrapper {
			...
			Init(){
				if (this.client == 0){
					this.asm := CLR_LoadLibrary(A_ScriptDir "\vigemwrapper.dll")
				}
			}
...
UpdateOverlay:
    if WinActive("ahk_exe destiny2.exe") {
        ...
        if (overlay.beginDraw()) {
            switch status {
                case 0:
                    overlay.drawText("Please ensure you are at The Landing inside The Pale Heart to Start | F4 Start | F5 Stop | F8 Exit | Thrallway.com", 10, 0, 32, 0xFFFFFFFF, "Courier", "olFF000000")
                case 1:
...
F4::
    ...
    ;; RUN AND LOOT CHEST
		...
		;; Aim Left, towards chest
        360Controller.Axes.RX.SetState(5)
        PreciseSleep(850)
        360Controller.Axes.RX.SetState(50)
        PreciseSleep(100)
        ;; Move Forward, towards chest
        360Controller.Axes.LY.SetState(100)
        PreciseSleep(200)
        360Controller.Axes.LY.SetState(50)
