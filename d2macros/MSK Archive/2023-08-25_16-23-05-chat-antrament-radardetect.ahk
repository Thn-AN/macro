...
class ViGEmTarget {
	...
	__New(){
		...
		if (this.Instance.OkCheck() != "OK"){
			msgbox,0x30,XP Script, ViGEmWrapper.dll failed to load!`n`nAsk for help here: https://discord.gg/KGyjysA5WY
			ExitApp
		}
	}
    ...
    if WinExist("ahk_exe destiny2.exe") {
        WinGetPos, x, y, w, h, ahk_exe destiny2.exe
			if (overlay.BeginDraw()) { 
				switch status {
					case 0:
						overlay.drawText("| F4 - Start | F5 - Reload | F8 - Close |", 10, 0, 32, 0xFFFFFFFF, "Courier")
					case 1:
						overlay.drawText("| Searching for red radar | F5 - Reload | F8 - Close |", 10, 0, 32, 0xFFFFFFFF, "Courier")
						overlay.DrawRectangle(100, 40, 60, 60, 0xFF00FF00, 1)
					case 2:
						overlay.drawText("| Red radar found | F5 - Reload | F8 - Close |", 10, 0, 32, 0xFFFFFFFF, "Courier")
				}
				overlay.EndDraw()
			}
...
F4::
	...
	Loop {
		...
		Loop 150 {   
			Loop {
				if (scan.PixelRegion(0x772D26, x+50, y+50, 160, 152)) {
					status = 2
					break
				} else {
					status = 1
