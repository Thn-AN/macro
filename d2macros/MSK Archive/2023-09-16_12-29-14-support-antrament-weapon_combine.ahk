...
UpdateOverlay:
	if WinExist("ahk_exe destiny2.exe") {
		...
		if (overlay.beginDraw()) { ; if we can draw the overlay... (is d2 active? did the class load correctly?)
			inputs := "| F8 - Close script | discord.gg/KGyjysA5WY |"
			switch status {
				case 0: ; script is not started
					overlay.drawText("| F3 - Setup script " . inputs, 10, 0, font_size, 0xFFFFFFFF, "Courier", "olFF000000")
				case 1: ; script is started
					overlay.drawText("| Setting up click #1 | F5 - Stop script/retry " . inputs . "`n| Please press number 1 on your keyboard to set your first click position |", 10, 0, font_size, 0xFFFFFFFF, "Courier" , "olFF000000")
				case 2:
					overlay.drawText("| Setting up click #2 | F5 - Stop script/retry " . inputs . "`n| Please press number 2 on your keyboard to set your second click position |", 10, 0, font_size, 0xFFFFFFFF, "Courier" , "olFF000000")
                case 3:
                    overlay.drawText("| Setting up click #3 | F5 - Stop script/retry " . inputs . "`n| Please press number 3 on your keyboard to set your third click position |`n| If you do not need a third click, just press 3 where you put click #2 |", 10, 0, font_size, 0xFFFFFFFF, "Courier" , "olFF000000")
                case 4:
                    overlay.drawText("| Setting up click #4 | F5 - Stop script/retry " . inputs . "`n| Please press number 4 on your keyboard to set your fourth click position |`n| If you do not need a fourth click, just press 4 where you put click #3 |", 10, 0, font_size, 0xFFFFFFFF, "Courier" , "olFF000000")
                case 5:
                    overlay.drawText("| All clicks now set. Please press F4 to replay the clicks. | F5 - Stop script/retry " . inputs, 10, 0, font_size, 0xFFFFFFFF, "Courier" , "olFF000000")
			}
...
Loop {
    ...
    Loop 4 {
        ...
        balls := 10 * sysx
        click
        dllcall("mouse_event", "uint", 0x8001, "uint", xposVar+balls, "uint", yposVar+balls)
        Sleep 10
        click
        dllcall("mouse_event", "uint", 0x8001, "uint", xposVar-balls, "uint", yposVar-balls)
        Sleep 10
