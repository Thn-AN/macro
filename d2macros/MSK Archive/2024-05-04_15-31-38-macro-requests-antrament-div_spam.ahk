...
InputBox, BindKey, Spam Bind, Please enter a bind to enable/disable spam mode, , 500, 125
if ErrorLevel {
    ExitApp
}
if (StrLen(BindKey) = 1) {
    ...
    MsgBox, The bind must be one character long, press OK to try again
    Reload
}
...
if WinExist("ahk_exe destiny2.exe") {
    WinGetPos, PosX, PosY, Width, Height, ahk_exe destiny2.exe
    if (Overlay.beginDraw()) {
        if (IsEnabled) {
            Overlay.drawText("Spam mode enabled (" . BindKey . ")", PosX+10, Height-40, 28, 0xFF00FF00, "Courier", "olFF000000")
        } else {
            Overlay.drawText("Spam mode disabled (" . BindKey . ")", PosX+10, Height-40, 28, 0xFFFF0000, "Courier", "olFF000000")
        }
        Overlay.endDraw()
    }
}
