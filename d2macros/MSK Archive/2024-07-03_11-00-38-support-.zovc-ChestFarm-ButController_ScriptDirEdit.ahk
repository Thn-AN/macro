...
class ViGEmWrapper {
	...
	CreateInstance(cls){
		try {
			...
			MsgBox,0x40,Antra's Background Phant Frags Script, You do not have ViGEm installed!
			...
			MsgBox,4,Antra's Background Phant Frags Script, The ViGEm installer is now in the same place as this script.`n`nPress yes if you need help!
			IfMsgBox, Yes
			{
...
class ViGEmTarget {
	...
	__New(){
		...
		if (this.Instance.OkCheck() != "OK"){
			msgbox,4,Antras Background Phant Frags Script, The .dll failed to load!`n`nPress yes if you need help!
			IfMsgBox, Yes
			{
				Run, https://discord.gg/KGyjysA5WY
				exitapp
...
UpdateOverlay:
	if WinExist("ahk_exe destiny2.exe") 
	{
		...
		{
			inputs := "| F8 - Close Script | F9 - Setup Info | Pale Heart Chest Farming - Zov edition |"
			switch status 
			{
...
Start_Route: 
	...
	Loop
	{
		...
		Loop 40
		{
			360Controller.Buttons.Back.SetState(true) 	; Z- Open Map
			PreciseSleep(1000)							; ↑
			360Controller.Buttons.Back.SetState(false)	; ↑
			PreciseSleep(250)
			360Controller.Axes.LX.SetState(0)			; Z- Stick to left
			PreciseSleep(2140)							; ↑
			360Controller.Axes.LX.SetState(50)			; ↑
			PreciseSleep(100)
			360Controller.Buttons.A.SetState(true)		; Z- Fast travel to The Landing
			PreciseSleep(1100)							; ↑
			360Controller.Buttons.A.SetState(false)		; ↑
			PreciseSleep(10000)
			360Controller.Axes.RX.SetState(10)			; Z- Initial left turn after spawning
			PreciseSleep(565)							; ↑
			360Controller.Axes.RX.SetState(50)			; ↑
			PreciseSleep(100)
			360Controller.Buttons.LS.SetState(true)		; Z- Sprint down
			PreciseSleep(50)
			360Controller.Axes.LY.SetState(100)				; Z- Walk forward
			PreciseSleep(7500)								; ↑
			360Controller.Axes.LY.SetState(50)				; Z- Stop Walking forward
			PreciseSleep(50)
			360Controller.Buttons.LS.SetState(false)	; Z- Sprint release
			PreciseSleep(100)
			360Controller.Axes.RX.SetState(90)			; Z- Turn right, toward chest
			PreciseSleep(2465)							; ↑
			360Controller.Axes.RX.SetState(50)			; ↑
			PreciseSleep(30000)
			360Controller.Buttons.LS.SetState(true)		; Z- Sprint down
			...
			360Controller.Axes.LY.SetState(100)				; Z- Walk forward
			PreciseSleep(3860)
			360Controller.Axes.RX.SetState(18)					; Z- Start left arc
			PreciseSleep(4250)									; ↑
			360Controller.Axes.RX.SetState(50)					; Z- Stop left arc
			PreciseSleep(1000)
			360Controller.Axes.LY.SetState(50)				; Z- Stop Walking forward
			PreciseSleep(50)
			360Controller.Buttons.LS.SetState(false)	; Z- Sprint release
			PreciseSleep(50)
			360Controller.Buttons.X.SetState(true)		; Z- Loot chest
			PreciseSleep(1350)							; ↑
		...
		360Controller.Buttons.Back.SetState(true)	; Z- Ghost menu
		PreciseSleep(100)							; ↑
		360Controller.Buttons.Back.SetState(false)	; ↑
		PreciseSleep(100)
		360Controller.Buttons.Y.SetState(true)		; Z- Return to orbit
		PreciseSleep(3200)							; ↑
		360Controller.Buttons.Y.SetState(false)		; ↑
		PreciseSleep(15000)
		360Controller.Buttons.Back.SetState(true)	; Z- Open Destinations tab
		PreciseSleep(100)							; ↑
		360Controller.Buttons.Back.SetState(false)	; ↑
		PreciseSleep(1000)
		360Controller.Axes.LY.SetState(70)			; Z- Adjust cursor because bungo bungelin
		PreciseSleep(50)							; ↑
		360Controller.Axes.LY.SetState(50)			; ↑
		PreciseSleep(50)							
		360Controller.Axes.LY.SetState(30)			; Z- Cursor down slightly
		PreciseSleep(50)							; ↑
		360Controller.Axes.LY.SetState(50)			; ↑
		PreciseSleep(600)
		360Controller.Buttons.A.SetState(true)		; Z- Select Pale Heart
		PreciseSleep(100)							; ↑
		360Controller.Buttons.A.SetState(false)		; ↑
		PreciseSleep(1000)
		360Controller.Axes.LX.SetState(0)			; Z- Cursor to left
		PreciseSleep(2120)							; ↑ 2140 default
		360Controller.Axes.LX.SetState(50)			; ↑
		PreciseSleep(200)
		360Controller.Buttons.A.SetState(true)		; Z- Select Landing
		PreciseSleep(100)							; ↑
		360Controller.Buttons.A.SetState(false)		; ↑
		PreciseSleep(100)
		360Controller.Axes.LX.SetState(100)			; Z- Cursor to the right
		360Controller.Axes.LY.SetState(28)			; Z- Cursor down, slowly
		PreciseSleep(990)
		360Controller.Axes.LY.SetState(50)			; Z- Cursor stops
		360Controller.Axes.LX.SetState(50)			; Z- Cursor stops
		PreciseSleep(100)
		360Controller.Buttons.A.SetState(true)		; Z- Hits Launch button
		PreciseSleep(100)							; ↑
		360Controller.Buttons.A.SetState(false)		; ↑
		PreciseSleep(30000)
...
Info_Hub:
MsgBox, , Setup Info, Start this macro from patrol in the Pale Heart.`nYou may need to clear Pathfinder objectives the cursor hits, if any, while moving on the map and any objectives related to opening chests in The Landing.`n`nThis macro is made for the 16:9 aspect ratio and should be locked to 30fps.`nUse the default controller binds and settings.`nNo other settings need to be changed.`n`nAvoid equipping weapons or exotic armors that affect your movement speed.`nYour Mobility stat does not matter for this macro.`n`nBest of luck out there! o/
