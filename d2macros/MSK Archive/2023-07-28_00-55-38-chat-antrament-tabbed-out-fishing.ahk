...
SearchForX:
    Loop {
        if (scan.pixelCountRegion(hex, x + startWidth, y + startHeight, regionWidth, regionHeight) < threshold) {
            fails++
            360Controller.buttons.y.setState(true) ; Simulate pressing the 'Y' button on the controller
            Sleep, 10
...
XFound:
    controller.buttons.x.setState(true) ; Simulate pressing the 'X' button on the controller
    Sleep 800
    controller.buttons.x.setState(false) ; Release the 'X' button
    if mod(num, 2) {
        fish++ ; Increment fish count if the 'num' variable is odd
    } else {
        Status = 2
...
; XNotFound subroutine - actions to perform when 'X' is not found within a time limit
XNotFound:
    FileAppend, 3000 fails!`n, fishinglog.txt ; Log the failure to find 'X'
    Status = 3
    axis := 10
    Loop {
        360Controller.Axes.LX.SetState(axis) ; Move the left analog stick along the X-axis
        360Controller.Axes.LY.SetState(axis) ; Move the left analog stick along the Y-axis
        Sleep 500
        axis += 10
        if (axis = 100){
            Sleep 1000
    ...
    360Controller.Axes.LX.SetState(50) ; Center the left analog stick along the X-axis
    360Controller.Axes.LY.SetState(50) ; Center the left analog stick along the Y-axis
    Loop {
        ; Jiggle the right analog stick to prevent AFK kick
        ...
        360Controller.Axes.LX.SetState(50)
        Sleep 50
        360Controller.buttons.x.setState(true) ; Simulate pressing the 'X' button on the controller
        Sleep 800
