...
F3:: ; main hotkey that runs the script
{
    ...
    loop, ; Orbit loop
    {
        ...
        loop, ; Run loop
        {
            ...
            PLAYER_DATA[CURRENT_GUARDIAN]["ClassStats"]["current_runs"]++
            PLAYER_DATA[CURRENT_GUARDIAN]["ClassStats"]["current_time"] += A_TickCount - CURRENT_FARM_START_TIME
            if (EXOTIC_DROP)
...
update_ui()
{
    ...
    ; Exotic Drop Rate
    current_exotic_drop_rate_ui.update_content("Exotic Drop Rate - " Round(PLAYER_DATA[CURRENT_GUARDIAN]["ClassStats"]["current_exotics"]/current_chest("pickups")*100,2) "%")
    ; Average Loop Time
    ...
    current_missed_chests_percent_ui.update_content("Percent Chests Missed - " Round(100 - (current_chest("pickups")/current_chest("appearances"))*100, 2) "%")
    ; Per chest stats
    ; -update_chest_ui()
    ...
    ; Exotic Drop Rate
    total_exotic_drop_rate_ui.update_content("Exotic Drop Rate - " Round((compute_total_stat("exotics")/total_chest("pickups")*100),2) "%")
    ; Average Loop Time
    total_average_loop_time_ui.update_content("Average Loop Time - " format_timestamp(compute_total_stat("time")/compute_total_stat("runs"), false, true, true, true, 2))
    ; Percent Chests Missed
    total_missed_chests_percent_ui.update_content("Percent Chests Missed - " Round(100 - (total_chest("pickups")/total_chest("appearances"))*100, 2) "%")
    ; Per chest stats
    ; -update_chest_ui()
