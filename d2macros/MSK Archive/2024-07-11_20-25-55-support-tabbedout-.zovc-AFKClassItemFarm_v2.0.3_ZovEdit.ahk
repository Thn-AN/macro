...
;; Start Farm Script stuff
	...
	;; Reset your instance of the Landing after 40 runs for consistency
		if(ResetCount >= 40 || ResetCount == -1) {
			ResetCount = 0
			if(inOrbit == 0) {							; Only runs this if you are not already in orbit
				360Controller.Buttons.RS.SetState(true) ; Z- Presses a button, that I dont think does anything, but acts as a sacrificial input to tell(or remind) destiny that we're a controller now
				PreciseSleep(100)
			...
			PreciseSleep(100)
			360Controller.Axes.LY.SetState(50)			; Z- Double stop, because bungo is the worst about eating inputs
			PreciseSleep(100)
			360Controller.Buttons.A.SetState(true)
			PreciseSleep(500)
			360Controller.Buttons.A.SetState(false)
	...
	;; RUN AND LOOT CHEST
		...
		;; Stop running
		360Controller.Axes.LY.SetState(50)
		360Controller.Buttons.LS.SetState(False)	; Z- Moved sprint end to after this run. some peeps weren't keeping the sprint toggle
