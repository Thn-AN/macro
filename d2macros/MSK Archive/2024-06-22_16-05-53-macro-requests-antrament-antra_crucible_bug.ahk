...
UpdateOverlay:
	if WinExist("ahk_exe destiny2.exe") {
		...
		if (overlay.beginDraw()) { ; if we can draw the overlay... (is d2 active? did the class load correctly?)
			inputs := "| F8 - Close script | Thrallway.com |"
			switch status {
				...
				case 1: ; script is started
					overlay.drawText("| Press 1 to set the launch button position !YOU MUST BE HOVERING OVER THE LAUNCH BUTTON! | F5 - Stop script/retry " . inputs, 10, 0, font_size, 0xFFFFFFFF, "Courier" , "olFF000000")
				case 2:
                    overlay.drawText("| Launch button position set! Press F4 to start | F5 - Stop script/retry " . inputs, 10, 0, font_size, 0xFFFFFFFF, "Courier" , "olFF000000")
				case 3:
                    overlay.drawText("| Farming now. | F5 - Stop script/retry " . inputs, 10, 0, font_size, 0xFFFFFFFF, "Courier" , "olFF000000")
			}
