...
Global
    ...
    bannerhexValues := ["0x522F64", "0x5B376D", "0x633E75", "0x69447B"]
    ...
    loop {
        loop 3 {
            loop 4 {
                if (scan.PixelRegion(bannerhexValues[A_Index], w/2, h/2, w/2, h/2, 3, &returnX, &returnY, "BTRL")) {
                    break outer
                } 
            }
        }
        msgbox "couldn't find the banners, ask for help @ thrallway.com, somethings wrong"
        Reload
    }
    break_outer:

    if (overlay.BeginDraw()) { 
        overlay.drawText("Buying banners until you press F5 | Thrallway.com", x+10, y+0, 40, 0xFFFFFFFF, "Courier", "olFF000000")
        overlay.EndDraw()
    }
    loop {
