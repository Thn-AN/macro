;####################
; AFK Movement Macro by UnLuckyLust
; This is a script that has been rebuilt and converted to use as a simple AFK macro with easy settings configuration
; base functions from https://thrallway.com/
;####################
...
class ViGEmWrapper {
    ...
    CreateInstance(cls){
        try {
            ...
            MsgBox,0x40,AFK Movement Script, You do not have ViGEm installed! Press OK to continue with downloading and installing ViGEm!
            ...
            MsgBox,3,AFK Movement Script,The ViGEm installer has been downloaded to %A_temp%.`n`nPress Yes to run the installer as admin!  Press No if you need help!`n`nPress Cancel or close the window to open the directory with the downloaded installer!
            IfMsgBox, Yes
            {
...
class ViGEmTarget {
    ...
    __New(){
        ...
        if (this.Instance.OkCheck() != "OK"){
            msgbox,4,AFK Movement Script, The .dll failed to load!`n`nPlease visit the #support channel in our discord for help resolving this.`n`nPress yes to launch our discord server for help!
            IfMsgBox, Yes
            {
                Run, https://thrallway.com/
                exitapp
...
hotkeyDefinitions := ["4:StartScript", "5:ReloadScript", "6:OpenSettings", "7:OpenHelp", "8:ExitApplication"]
...
getOverlayText(status) {
    ...
    for k, v in hotkeys {
      if (v = "StartScript") {
          ...
          settingsHotkey := translateHotkey(k)
      } else if (v = "OpenHelp") {
          helpHotkey := translateHotkey(k)
    ...
    if (status == 0) {
        return "" startHotkey " - Start AFK Movement `n" settingsHotkey " - Open Settings `n" helpHotkey " - Help `n" exitHotkey " - Exit"
    } else if (status = 1) {
        return "You can tab out now! `n" stopHotkey " - Stop/Reload `n" settingsHotkey " - Open Settings `n" exitHotkey " - Quit Movement Script"
    }
}
...
getMsgBoxText(settings_data) {
    ...
    return "Script Version: " object(settings_data*)["Version"] " `n`nCurrent Script Settings are listed below,`nYou can change the values in the settings menu.`n`nMove Right            - " getStatus(object(settings_data*)["MoveRight"]) "`nMove Down           - " getStatus(object(settings_data*)["MoveDown"]) "`nMove Left               - " getStatus(object(settings_data*)["MoveLeft"]) "`nMove Up                - " getStatus(object(settings_data*)["MoveUp"]) "`nActivate Melee       - " getStatus(object(settings_data*)["ActivateMelee"]) "`nActivate Grenade  - " getStatus(object(settings_data*)["ActivateGrenade"]) "`nActivate Super       - " getStatus(object(settings_data*)["ActivateSuper"]) "`n`nListed below are all the script shortcut keys:`n`n" startHotkey " - Start AFK Movement`n" stopHotkey " - Reload/Stop`n" settingsHotkey " - Open Settings`n" helpHotkey " - Help`n" exitHotkey " - Quit/Exit`n`nAre you Ready to start?`nYES = Start the AFK Macro `nNO = Change script Settings"
}


; Function to display the initial msgbox
ShowInitialMsgBox(settings_data)
{
    Loop
    {
        msgBoxText := getMsgBoxText(settings_data)
        MsgBox, 4, AFK Movement - Info, %msgBoxText%
        IfMsgBox Yes
        {
        ...
        Else
        {
            ; Open the settings GUI if No is pressed
            gosub, OpenSettings
            return
        }
    }
...
startup:
    settingsini := A_ScriptDir . "\AFKMovement.ini"
    ; Check if the "AFKMovement.ini" file exists.
    if fileexist(settingsini) {
        ; If the file exists, load settings from it using a subroutine.
        gosub, readfromini
        if(cleanupoldshit205 != 1){
            reset_to_default = 1
...
inisetup:
    ...
    gosub, writetoini		; Save the default settings to "AFKMovement.ini" file.
    gosub, readfromini		; Load settings from the "AFKMovement.ini" file.
    
    settings_data := ["Version",version, "MoveRight",MoveRight, "MoveDown",MoveDown, "MoveLeft",MoveLeft, "MoveUp",MoveUp, "ActivateMelee",ActivateMelee, "ActivateGrenade",ActivateGrenade, "ActivateSuper",ActivateSuper]
    ShowInitialMsgBox(settings_data)
return

writetoini:
    iniwrite, %MoveRight%, %settingsini%, script_settings, MoveRight
    iniwrite, %MoveRightAmount%, %settingsini%, script_settings, MoveRightAmount
    iniwrite, %MoveDown%, %settingsini%, script_settings, MoveDown
    iniwrite, %MoveDownAmount%, %settingsini%, script_settings, MoveDownAmount
    iniwrite, %MoveLeft%, %settingsini%, script_settings, MoveLeft
    iniwrite, %MoveLeftAmount%, %settingsini%, script_settings, MoveLeftAmount
    iniwrite, %MoveUp%, %settingsini%, script_settings, MoveUp
    iniwrite, %MoveUpAmount%, %settingsini%, script_settings, MoveUpAmount
    iniwrite, %ActivateMelee%, %settingsini%, script_settings, ActivateMelee
    iniwrite, %ActivateGrenade%, %settingsini%, script_settings, ActivateGrenade
    iniwrite, %ActivateSuper%, %settingsini%, script_settings, ActivateSuper

    iniwrite, %WaitRandomMin%, %settingsini%, script_settings, WaitRandomMin
    iniwrite, %WaitRandomMax%, %settingsini%, script_settings, WaitRandomMax

    iniwrite, %cleanupoldshit205%, %settingsini%, script_settings, cleanupoldshit205

    gui, destroy
return

readfromini:
    iniread, MoveRight, %settingsini%, script_settings, MoveRight, %A_Space%
    iniread, MoveRightAmount, %settingsini%, script_settings, MoveRightAmount, %A_Space%
    iniread, MoveDown, %settingsini%, script_settings, MoveDown, %A_Space%
    iniread, MoveDownAmount, %settingsini%, script_settings, MoveDownAmount, %A_Space%
    iniread, MoveLeft, %settingsini%, script_settings, MoveLeft, %A_Space%
    iniread, MoveLeftAmount, %settingsini%, script_settings, MoveLeftAmount, %A_Space%
    iniread, MoveUp, %settingsini%, script_settings, MoveUp, %A_Space%
    iniread, MoveUpAmount, %settingsini%, script_settings, MoveUpAmount, %A_Space%
    iniread, ActivateMelee, %settingsini%, script_settings, ActivateMelee, %A_Space%
    iniread, ActivateGrenade, %settingsini%, script_settings, ActivateGrenade, %A_Space%
    iniread, ActivateSuper, %settingsini%, script_settings, ActivateSuper, %A_Space%

    iniread, WaitRandomMin, %settingsini%, script_settings, WaitRandomMin, %A_Space%
    iniread, WaitRandomMax, %settingsini%, script_settings, WaitRandomMax, %A_Space%

    iniread, cleanupoldshit205, %settingsini%, script_settings, cleanupoldshit205, %A_Space%
...
;    Menu, MyMenuBar, Add, Help, OpenHelp
;    Menu, MyMenuBar, Add, Reset to Default, defaultsettings
;    Gui, Menu, MyMenuBar

    gui font, s14 bold q5 cblack, Courier New
    gui add, button, x5 y257 w120 h30 +default, Save
    gui add, button, x125 y257 w120 h30 gdefaultsettings, Reset
    gui add, groupbox, x5 y0 w240 h255, Movement Settings

    gui font, s11 norm q5 cblack, verdana
    gui add, text, x15 YP+25 w135 h25 +0x200 , Min Wait Duration
    gui add, edit, x157 YP+0 w80 h25 vWaitRandomMin, %WaitRandomMin%
    
    gui add, text, x15 YP+25 w135 h25 +0x200, Max Wait Duration
    gui add, edit, x157 YP+0 w80 h25 vWaitRandomMax, %WaitRandomMax%

    gui add, checkbox, x15 YP+30 w30 checked%MoveRight% vMoveRight
    gui add, text, x45 YP-5 w135 h25 +0x200, Move Right
    gui add, edit, x157 YP+0 w80 h25 vMoveRightAmount, %MoveRightAmount%

    gui add, checkbox, x15 YP+30 w30 checked%MoveDown% vMoveDown
    gui add, text, x45 YP-5 w135 h25 +0x200, Move Down
    gui add, edit, x157 YP+0 w80 h25 vMoveDownAmount, %MoveDownAmount%
    
    gui add, checkbox, x15 YP+30 w30 checked%MoveLeft% vMoveLeft
    gui add, text, x45 YP-5 w135 h25 +0x200, Move Left
    gui add, edit, x157 YP+0 w80 h25 vMoveLeftAmount, %MoveLeftAmount%
    
    gui add, checkbox, x15 YP+30 w30 checked%MoveUp% vMoveUp
    gui add, text, x45 YP-5 w135 h25 +0x200, Move Up
    gui add, edit, x157 YP+0 w80 h25 vMoveUpAmount, %MoveUpAmount%
    
    gui add, checkbox, x15 YP+30 w30 checked%ActivateMelee% vActivateMelee
    gui add, text, x45 YP-5 w135 h25 +0x200, Activate Melee
    
    gui add, checkbox, x15 YP+30 w30 checked%ActivateGrenade% vActivateGrenade
    gui add, text, x45 YP-5 w135 h25 +0x200, Activate Grenade
    
    gui add, checkbox, x15 YP+30 w30 checked%ActivateSuper% vActivateSuper
    gui add, text, x45 YP-5 w135 h25 +0x200, Activate Super

    gui show, w250, AFK Movement - Settings
return
...
defaultsettings:
    MsgBox,4,AFK Movement Script,Do you want to reset your settings to default?.`n`nPress YES to reset `nPress NO to cancel
    IfMsgBox, Yes
    {
        reset_to_default = 1
        gui, destroy
...
UpdateOverlay:
    if WinActive("ahk_exe destiny2.exe") {
        ...
        if (overlay.beginDraw()) {
            ...
            overlay.drawText(overlayText, 10, 5, 18, 0xFFFFFFFF, "Courier New", "olFF000000")
            overlay.endDraw()
        } else {
            overlay := new ShinsOverlayClass("ahk_exe destiny2.exe")
        }
...
StartScript:
  Status = 1
  Random, stopInterval, WaitRandomMin, WaitRandomMax
  ...
  Loop {
    if (MoveRight) {
      360Controller.Axes.LX.SetState(100)       ; Move right
      Sleep, MoveRightAmount
      360Controller.Axes.LX.SetState(50)        ; Stop
      Sleep, stopInterval
    }
    if (MoveDown) {
      360Controller.Axes.LY.SetState(0)         ; Move down
      Sleep, MoveDownAmount
      360Controller.Axes.LY.SetState(50)        ; Stop
      Sleep, stopInterval
    }
    if (MoveLeft) {
      360Controller.Axes.LX.SetState(0)         ; Move left
      Sleep, MoveLeftAmount
      360Controller.Axes.LX.SetState(50)        ; Stop
      Sleep, stopInterval
    }
    if (MoveUp) {
      360Controller.Axes.LY.SetState(100)       ; Move up
      Sleep, MoveUpAmount
      360Controller.Axes.LY.SetState(50)        ; Stop
      Sleep, stopInterval
    }
    if (ActivateMelee) {                        ; Activate Melee
      360Controller.Buttons.RB.SetState(true)
      Sleep, 100
      360Controller.Buttons.RB.SetState(false)
      Sleep, stopInterval
    }
    if (ActivateGrenade) {                        ; Activate Grenade
      360Controller.Buttons.LB.SetState(true)
      Sleep, 100
      360Controller.Buttons.LB.SetState(false)
      Sleep, stopInterval
    }
    if (ActivateSuper) {                        ; Activate Super
      360Controller.Buttons.LB.SetState(true)
      360Controller.Buttons.RB.SetState(true)
      Sleep, 100
      360Controller.Buttons.LB.SetState(false)
...
; Open Settings Label
OpenSettings:
  	if (winexist("AFK Movement - Settings")) {
		gui, show
	} else {
		gosub, settingsgui
	}
