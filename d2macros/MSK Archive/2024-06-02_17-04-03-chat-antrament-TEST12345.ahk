...
if !WinExist("ahk_exe destiny2.exe"){
    msgbox "Destiny 2 isn't open lol"
    Exitapp
} else {
    WinWait "Destiny 2"
    WinMove 0, 0
...
Global overlay := ShinsOverlayClass.call("ahk_exe destiny2.exe")
...
Global
    ...
    phantfragshexValues := ["0x3767B2", "0x3F70B8", "0x4C7CBF", "0x507FBD"]

    if (overlay.BeginDraw()) { 
        overlay.drawText("Trying to find what you want to buy... | F8 to exit", x+10, y+0, 40, 0xFFFFFFFF, "Courier", "olFF000000")
		overlay.DrawRectangle(w/2, h/2, w/2, h/2 0xFFFF0000, 3)
        overlay.EndDraw()
    }
    outer:
    loop {
        loop 3 {
            loop 4 {
                if (scan.PixelRegion(phantfragshexValues[A_Index], w/2, h/2, w/2, h/2, 3, &returnX, &returnY)) {
                    break outer
                } 
            }
        }
        msgbox "couldn't find the phant frags, ask for help @ thrallway.com, somethings wrong"
        Reload
    }
    break_outer:

    if (overlay.BeginDraw()) { 
        overlay.drawText("Buying phant frags until you press F5 | Thrallway.com", x+10, y+0, 40, 0xFFFFFFFF, "Courier", "olFF000000")
        overlay.EndDraw()
    }
    loop {
        dllcall("mouse_event", "uint", 0x8001, "uint", (returnX-5)*sysx, "uint", (returnY-5)*sysy) 
        Sleep(50)
