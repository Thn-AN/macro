...
; The main function of this script was made by kielch/keelnode, thanks to Orchrist for making it useful again!
;####################
...
; Example, change ^4 in "^4:StartScript" to +4 becomes "+4:StartScript" which is Shift & 5 ..... or change it to F5 becomes F5:StartScript so the keybind is F5


; Define hotkeys and their labels in a single place - 
hotkeyDefinitions := ["4:StartScript", "5:ReloadScript", "8:ExitApplication"]

;------------------------------------------------- HOTKEYS ABOVE, just change the bit after " and before : -----------------------------------


; Initialize hotkeys associative array (the top defs)
hotkeys := {}
...
Loop, % hotkeyDefinitions.MaxIndex()
{
    ...
    StringSplit, hotkeyParts, hotkeyDef, `: ;not sure why StringSplit gets highlighted here but it works
    hotkey := hotkeyParts1
    action := hotkeyParts2
    ...
    ; Populating the hotkeys array
    hotkeys[hotkey] := action
}

; Function to translate hotkey symbols to readable names - only tested several cos too tired to add more, and these were commonly used
...
; Function to generate msgbox text based on hotkeys
getMsgBoxText() {
    global hotkeys
    startHotkey := ""
    stopHotkey := ""
    ...
    return "BE IN ORBIT BEFORE STARTING THE MACRO `n ......................................... `n `n- Private Match Crucible`n- Control`n- Javelin-4 `n- 5/10 mins - 10 mins gives more REP`n`n- Start the script when you are at the GREEN LAUNCH BUTTON Screen`n `n ......................................... `n  `nIf you want to change KEYBINDS below, check ~Line 2203 in the code under HOTKEYS section `n  `n " startHotkey "   to Start`n " stopHotkey "   to Reload/Stop script`n " exitHotkey "   to Quit/Exit script `n`n READY? Press Yes then Start the macro with the keybind"
}

; Initial call to show the msgbox
ShowInitialMsgBox()

; Function to display the initial msgbox
ShowInitialMsgBox()
{
    Loop
    {
...
;##################################################### OLD Hotkeys - DO NOT TOUCH  !!! They might break stuff idk, but for a future project I leaving it here.. ty

; F4::Gosub, StartScript
; F5::Reload
; F8::ExitApp
...
StartScript:
    ...
    Loop {
        ...
        Sleep, 500
        360Controller.Axes.LY.SetState(0)  ; Move down to max
        Sleep, 500
