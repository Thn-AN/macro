...
F4::
	InputBox, UserInput, Antra's Phantasmal Fragment Script v1.0, Please enter how many Phantasmal Frags you want `n(put like 999999 if you want as many as possible), , ,140
	if WinExist("Destiny 2") {
		WinActivate
	} else {
		msgbox, Destiny 2 not found
	}
	if (!UserInput) {
		reload
	}
	Loop %UserInput% { ; 100 is a reasonable amount of casts to do before going back to the main loop to move backwards again for anti-afk
		status = 1
		360Controller.Buttons.A.SetState(true) ; throw grenade
		Sleep 300 ; this delay is for both anti-afk every 100 casts AND allowing the ability to be cast (ability sometimes does not cast if no delay between button set true and set false)
		360Controller.Buttons.A.SetState(false) ; stop throwing melee
		Sleep 500
	}
	status = 2
Return
