...
SendMode Input
		...
		; ============================================================================================
		;										Hotkeys Info
		;	F4 to change settings
		;	F5 Initialize first Launch
		;	F5 Relaunch on mission complete
		;	F6 to resume CP farm on team full
		;	F6 Toggle script Pause
		;	F7 Toggle Help / Hide this panel
		;	F8 Exit script

		; ============================================================================================
		; 									Hotkeys & Initial setup
			SetTimer, UpdateOverlay,  2500
			gosub, UpdateOverlay
			OverlayHotkeys := true
			status = 2
			return
					...
					if (overlay.beginDraw()) {
						switch status {
							case 0:
								overlay := ""
							case 1:
								overlay.drawText("F6 when ready to Launch.				", 1860, 905, 32, 0xFFD5C659, "Courier", "olFF000000")
								overlay.drawText("Tell team to join now.				", 1860, 940, 32, 0xFFD5C659, "Courier", "olFF000000")
							case 2:
								overlay.drawText("F4 Ativity setup 						", 10,   0,   32, 0xFFD5C659, "Courier", "olFF000000")
								overlay.drawText("F5 Initialize first Launch			", 10,   35,  32, 0xFFD5C659, "Courier", "olFF000000")
								overlay.drawText("F5 Relaunch on mission complete		", 10,   70,  32, 0xFFD5C659, "Courier", "olFF000000")
								overlay.drawText("F6 Toggle Pause						", 10,   105, 32, 0xFFD5C659, "Courier", "olFF000000")
								overlay.drawText("F7 Toggle Help						", 10,   140, 32, 0xFFD5C659, "Courier", "olFF000000")
								overlay.drawText("F8 Exit 								", 10,   175, 32, 0xFFD5C659, "Courier", "olFFA03E40")
						}
						overlay.endDraw()
					} else {
...
[S24] Battleground: Echos
[S24] Battleground: Echos (Expert)
[S24] Arena: Breach Executable (Normal)
[S24] Arena: Breach Executable (Expert)
[S24] Enigma Protocol
[S24] Enigma Protocol (Private)

[Event] Solstice - Bonfire Bash
	)
	,,270,923 							; Infobox size - 1 line ~18px

	InputBox, LengthStr, Farm type,
	(
Checkpoint [cp] or full[f]?
Leave Empty for full
When selected, F5 to Launch activity
	),,250,160
	if (LengthStr == "Fulls" || LengthStr == "fulls" || LengthStr == "full" || LengthStr == "f") {
		Length := 1
	} else if (LengthStr == "Checkpoints" || LengthStr == "checkpoints" || LengthStr == "CPs" || LengthStr == "cps" || LengthStr == "cp" || LengthStr == "c") {
		InputBox, Host, Host selection, Which character do you have CP on (1-3 slot)?,,200,150
		InputBox, Farm, Farm selection, Which character do you want to farm on (1-3 slot)?,,200,150
		MsgBox,
	(
Press F5 to start
Press F5 on mission complete
Press F6 when all people join in orbit,
when Launch button is highlighted

	! Have friend join ID copied !
	)
	status = 0
		Length := 0
	} else {
	...
	switch Activity {
		...
		case "Prophecy", "prophecy", "proph", "prop":
		fastclick(1589, 205, 20, 1)
		PreciseSleep(1200)
		fastclick(1650, 300, 500, 1)
		Launch(Length)
		...
		case "Ghosts of the Deep (Normal)", "ghosts", "gotd", "ghost", "rybki":
		fastclick(1589, 205, 20, 1)
		PreciseSleep(1200)
		fastclick(900, 300, 2000, 1)
		Launch(Length)

		case "Ghosts of the Deep (Master)", "master ghosts", "mgotd", "mghost", "mrybki":
		fastclick(1589, 205, 20, 1)
		PreciseSleep(1200)
		fastclick(900, 300, 2000, 1)
		PreciseSleep(700)
		...
		SelectNodeLegends()
		PreciseSleep(1200)
		fastclick(550, 720, 500, 1)
		Launch(Length)

		case "Vault of Glass (Master)", "master vog", "mvog":
		SelectNodeLegends()
		PreciseSleep(1200)
		fastclick(615, 550, 720, 1)
		PreciseSleep(700)
		fastclick(2178, 1114, 20, 1)
		PreciseSleep(1100)
		fastclick(442, 460, 20, 1)
		...
		case "Salvation's Edge (Normal)", "salv", "salva", "salvation", "salvations", "edge", "salvation edge", "salvations edge", "se":
		fastclick(1280, 650, 20, 1)
		fastclick(2559, 150, 20, 1)
		PreciseSleep(4625)
		fastclick(1280, 1000, 20, 1)
		Launch(Length)

		case "Master Salvation's Edge (Normal)", "msalv", "msalva", "msalvation", "msalvations", "medge", "msalvation edge", "msalvations edge", "mse":
		fastclick(1280, 650, 20, 1)
		fastclick(2559, 150, 20, 1)
		PreciseSleep(4625)
		fastclick(1280, 1000, 20, 1)
		...
		case "Vanguard Ops", "strikes", "strike", "ops", "v":
		fastclick(965, 201, 20, 1)
		PreciseSleep(1100)
		fastclick(1275, 665, 500, 1)
		Launch(Length)

		case "Grandmaster Nightfall", "gms", "gm":
		fastclick(965, 201, 20, 1)
		PreciseSleep(1100)
		fastclick(1638, 1068, 500, 1)
		Launch(Length)

		case "Dares of Eternity (Legend)", "doe", "dares", "kon":
		...
		case "Exotic Mission Rotator (Normal)", "exotic mission", "exotic", "rotator", "exo":
		fastclick(1589, 205, 20, 1)
		PreciseSleep(1200)
		fastclick(1280, 1190, 500, 1)
		Launch(Length)

		case "Exotic Mission Rotator (Legend)", "legend exotic mission", "legend rotator", "mexo":
		fastclick(1589, 205, 20, 1)
		PreciseSleep(1200)
		fastclick(1280, 1190, 500, 1)
		PreciseSleep(700)
		...
		case "The Whisper", "the whisper", "whisper", "tw":
		fastclick(1589, 205, 20, 1)
		PreciseSleep(1200)
		fastclick(750, 1100, 500, 1)
		Launch(Length)

		case "Master The Whisper", "master the whisper", "master whisper", "mtw":
		fastclick(1589, 205, 20, 1)
		PreciseSleep(1200)
		fastclick(750, 1100, 500, 1)
		PreciseSleep(700)
		...
		case "Zero Hour", "zero hour", "zero", "hour", "zh":
		fastclick(1589, 205, 20, 1)
		PreciseSleep(1200)
		fastclick(1811, 1100, 500, 1)
		Launch(Length)

		case "Master Zero Hour", "master zero hour", "mzero", "mhour", "mzh":
		fastclick(1589, 205, 20, 1)
		PreciseSleep(1200)
		fastclick(1811, 1100, 500, 1)
		PreciseSleep(700)
		...
		case "Battleground: Echos", "bg":
		SelectNodeHELM()
		PreciseSleep(1200)
		fastclick(860, 525, 500, 1)
		Launch(Length)

		case "Battleground: Echos (Expert)", "mbg":
		SelectNodeHELM()
		PreciseSleep(1200)
		fastclick(1720, 525, 500, 1)
		Launch(Length)

		case "Arena: Breach Executable (Normal)", "arena", "breach", "exec", "execut", "arena exec", "abe" :
		SelectNodeHELM()
		PreciseSleep(1200)
		fastclick(780, 800, 500, 1)
		Launch(Length)

		case "Arena: Breach Executable (Expert)", "marena", "mbreach", "mexec", "mexecut", "marena exec", "mabe" :
		SelectNodeHELM()
		PreciseSleep(1200)
		fastclick(780, 800, 500, 1)
		PreciseSleep(700)
		...
		case "Enigma Protocol", "enigma":
		SelectNodeHELM()
		PreciseSleep(1200)
		fastclick(1790, 800, 500, 1)
		Launch(Length)

		case "Bonfire Bash", "bonfire bash", "bonfire", "bask", "solstice", "bb", "sbb", "event", "Event":
		SelectNodeTower()
		PreciseSleep(1200)
		fastclick(1300, 320, 500, 1)
		Launch(Length)
	...
	; Change above coords if GUI is updated. Below is just summary.
