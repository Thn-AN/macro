...
class ViGEmTarget {
	...
	__New(){
		...
		if (this.Instance.OkCheck() != "OK"){
			msgbox,4,Antra's Background Phant Frags Script, The .dll failed to load!`n`nPress yes if you need help!
			IfMsgBox, Yes
			{
				Run, https://discord.gg/KGyjysA5WY
				exitapp
		...
		if (overlay.beginDraw()) { ; if we can draw the overlay... (is d2 active? did the class load correctly?)
			...
			switch status {
				...
				case 1: ; script is started
					overlay.drawText("| You may now tab out | F5 - Stop Script " . inputs . "`n| You should totally join that Discord... lots of cool Macros there... |`n| Also kisses... maybe. |", 10, 0, font_size, 0xFFFFFFFF, "Courier" , "olFF000000")
				case 2: ; script is started
					overlay.drawText("| All done! | F5 - Stop Script " . inputs . "`n| You should now have " . UserInput . " Extra Fragments |", 10, 0, font_size, 0xFFFFFFFF, "Courier" , "olFF000000")
			}
...
get_user_input:
Gui, add, Text,, Enter glimmer:
Gui, Add, Edit, vglimmer
Gui, add, Text,, Enter height:
Gui, Add, Edit, vprisms
...
submit:
Gui, Hide
GuiControlGet, glimmer
GuiControlGet, prisms
; given glimmer amount and prisms, how many blues can they buy?
; will they need to buy more glimmer from rahool? when? how many times?
; 
gosub, get_blue_armor
return


;assumes user is outside of all menus
get_blue_armor:
    ...
    360Controller.Dpad.SetState("Right")
    PreciseSleep(200)
    360Controller.Dpad.SetState("None")
    PreciseSleep(200)
    Loop %UserInput% {
	...
	gosub, upgrade_and_dismantle_blue_armor

return

;assumes user is outside of all menus and has blue chest armor
upgrade_and_dismantle_blue_armor:
    360Controller.Buttons.Start.SetState(true)
    PreciseSleep(100)
    360Controller.Buttons.Start.SetState(false) 
    PreciseSleep(900)
    ...
    Loop %UserInput% {
        ...
        360Controller.Axes.LY.SetState(60)
        PreciseSleep(1100)
        360Controller.Axes.LX.SetState(50)
...
;assumes user is stood directly in front of rahool, outside of all menus
get_glimmer_from_rahool:
; open rahool
; open materials exchange
; purchase glimmer with legendary shards
; back out, twice
; given the new amount of glimmer, calculate how many blue armors can be upgraded and go back to "get_blue_armor" and "upgrade_and_dismantle_blue_armor" as many times as possible
